  ...
  <div>
    ...
    >
      ...
      <el-table-column label="類型" width="110" align="center">
        <template #default="{ row }">
          ...
          <el-tag v-else-if="row.discountType === 'fixed'" type="warning"
            >固定金額</el-tag
          >
          <el-tag v-else-if="row.discountType === 'lottery'" type="danger"
            >抽獎活動</el-tag
          >
      ...
      <el-table-column label="優惠內容" width="120" align="center">
        <template #default="{ row }">
          <span v-if="row.discountType === 'percentage'"
            >{{ row.discountValue }}%</span
          >
          <span v-else-if="row.discountType === 'fixed'"
            >${{ row.discountValue }}</span
          >
          <span
            v-else-if="row.discountType === 'lottery'"
            style="color: #f56c6c"
            >抽獎</span
          >
          <span v-else>-</span>
      ...
      <el-table-column label="狀態" width="100" align="center">
        <template #default="{ row }">
          ...
          <el-tag v-else-if="isUpcomingPromotion(row)" type="info"
            >未開始</el-tag
          >
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="110px">
        ...
        <el-form-item v-if="form.discountType === 'lottery'" label="抽獎資格">
          <el-input
        ...
        <el-form-item label="每人使用次數">
          <el-input-number
            ...
            :precision="0"
            :placeholder="
              form.discountType === 'lottery' ? '每人可抽次數' : '每人使用次數'
            "
            style="width: 200px"
          />
          <span style="margin-left: 8px; color: #909399">
            ({{ form.discountType === "lottery" ? "每人可抽次數" : "次" }}，0
            表示無限制)
          </span>
        </el-form-item>
...
async function fetchList() {
  ...
  try {
    ...
    total.value = list.value.length; // 沒有 total，由 list 長度決定
  } catch (error) {
