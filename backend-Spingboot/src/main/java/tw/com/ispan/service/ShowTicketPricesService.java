...
import tw.com.ispan.entity.MovieBean;
import tw.com.ispan.entity.ScreenBean;
import tw.com.ispan.entity.ShowBean;
import tw.com.ispan.entity.ShowTicketPricesBean;
...
LocalTime endTime = show.getEndTime() != null
        ...
        // 2. 取得 movie（拿片長）
        MovieBean movie = show.getMovie(); // 如果你是用 movieId，而不是 ManyToOne，就改成 movieRepository.findById(show.getMovieId())...
        if (movie == null) {
            movie = movieRepository.findById(show.getMovie().getId())
                    .orElseThrow(() -> new EntityNotFoundException("找不到對應的 movie"));
        }

        Integer runtimeMinutes = movie.getRuntimeMinutes(); // e.g. movies.runtime_minutes

        // 3. 取得 screen（拿廳別價格）
        ScreenBean screen = show.getScreen(); // 同上，如果你只存 screenId 就改成 screenRepository.findById(show.getScreenId())...
        if (screen == null) {
            screen = screenRepository.findById(show.getScreen().getId())
                    .orElseThrow(() -> new EntityNotFoundException("找不到對應的 screen"));
        }
