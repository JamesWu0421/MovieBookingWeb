...
public class ReportService {
    ...
    public Map<String, Object> getComprehensiveSalesReport(
        ...
        report.put("ticketTypeDistribution", getTicketTypeDistribution(startDate, endDate, movieIds, screenIds, ticketTypes));
        report.put("screenTypeDistribution", getScreenTypeDistribution(startDate, endDate, movieIds, screenIds, ticketTypes));
    ...
    private Map<String, Object> getSalesOverview(
        ...
        return jdbcTemplate.queryForObject(sql.toString(), params.toArray(), (rs, rn) -> {
            ...
            data.put("averageTicketPrice",
                    totalTickets > 0 ?
                    totalRevenue.divide(new BigDecimal(totalTickets), 2, RoundingMode.HALF_UP) :
                    BigDecimal.ZERO);

            long days = ChronoUnit.DAYS.between(startDate, endDate) + 1;
            data.put("dailyAvgRevenue",
                    days > 0 ?
                    totalRevenue.divide(new BigDecimal(days), 2, RoundingMode.HALF_UP) :
                    BigDecimal.ZERO);
